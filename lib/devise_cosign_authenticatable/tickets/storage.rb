  ...
  module Tickets
    module Storage
      class AbstractTicketStore
        ...
        def process_single_sign_out(st)
          ...
          if session
            ...
            log.debug("Data for session #{session_id.inspect} was not found. It may have already been cleared by a local COSIGN logout request.")
          end

          if session_id
            log.info("Single-sign-out for service ticket #{session_id.inspect} completed successfuly.")
          else
            log.debug("No session id found for COSIGN ticket #{st}")
          end
        end
        ...
        def session_id_from_controller(controller)
          ...
          raise COSIGNClient::COSIGNException, "Failed to extract session_id from controller" if session_id.nil?
          session_id
        end
      end
      ...
      class LocalDirTicketStore < AbstractTicketStore
        ...
        def store_service_session_lookup(st, controller)
          raise COSIGNException, "No service_ticket specified." if st.nil?
          raise COSIGNException, "No controller specified." if controller.nil?
        ...
        def save_pgt_iou(pgt_iou, pgt)
          raise COSIGNException, "Invalid pgt_iou" if pgt_iou.nil?
          raise COSIGNException, "Invalid pgt" if pgt.nil?
        ...
        def retrieve_pgt(pgt_iou)
          raise COSIGNException, "No pgt_iou specified. Cannot retrieve the pgt." unless pgt_iou
          ...
          raise COSIGNException, "Invalid pgt_iou specified. Perhaps this pgt has already been retrieved?" unless pgt
